          ...
          <View style={globalStyles.e_layout_container}>
            ...
            {itemOrdered ? (
              <View style={(globalStyles.e_layout, styles.orderSuccess)}>
                ...
                <Text style={styles.IngredientsText}>{i18n.t('dish')}: {dish.title}</Text>
              ...
              <View style={(globalStyles.e_layout, styles.orderSuccess)}>
                <H1 style={styles.HeadingText}>{i18n.t('ordercantbeplaced')}</H1>
                <H2 style={styles.SubHeadingText}>{i18n.t('ordercantbeplaced_txt')}</H2>
              </View>
            ) : (
              <>
                <View style={globalStyles.e_layout}>
                  <SharedElement id={`item.${dish.id}.title`}>
                    <H1 style={styles.HeadingText}>{dish.title.split(/–|-/, 2)[lang_i].trim()}</H1>
                  </SharedElement>
                  {/*<Text style={styles.description}>{dish.description}</Text> */}
                  <H3 style={styles.SubHeadingText}>{i18n.t('ingredients')}</H3>
                  ...
                  {props.company.selectedCompany.type === "payment_by_user" ? (
                      ...
                      <View style={styles.priceWrap}>
                        <View style={{ flexDirection: "row" }}>
                          <View style={{ width: '100%', flexDirection: "row", marginTop: 10}}>
                            <TextInput editable={!loading} textAlign={"center"} style={styles.promo} placeholder={i18n.t('promocode')} placeholderTextColor="#B0D0CE" returnKeyType="done" value={promo} onChangeText={(e) => setPromo(e)} />
                    ...
                    <TouchableOpacity style={globalStyles.mainButton} onPress={() => (!loading ? chooseDish() : {})}>
                      {!loading ? <Text style={globalStyles.mainButtonText}>{i18n.t('selectthisdish')}</Text> : <ActivityIndicator size={"small"} color="#000" />}
                    </TouchableOpacity>
                  )}
                </View>
              </>
